<?php
/**
 * Финальное исправление кодировки - радикальное решение
 */

echo "<h1>🔧 ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ КОДИРОВКИ</h1>";
...
try {
  ...
  // Полностью правильные данные статьи
  $correct_data = [
...
<p>Помните, что психология – это нечто большее, чем просто наука. Это ключ к пониманию себя и других, к гармонии внутри и вокруг нас. Решение любой проблемы начинается с осознания ее существования. Не стесняйтесь обращаться за помощь и помните, что каждый шаг к изменениям приближает вас к лучшей жизни.</p>',
  ...
  // Находим AI-статьи
  $sql = "SELECT id FROM articles WHERE author = 'AI Assistant' ORDER BY created_at DESC LIMIT 1";
  ...
  echo "<h2>📄 Исправляем статью ID: {$article_id}</h2>";
  ...
  if ($result) {
    ...
    // Проверяем результат
    $check_sql = "SELECT title, content, excerpt FROM articles WHERE id = :id";
    $check_stmt = $pdo->prepare($check_sql);
    $check_stmt->execute([':id' => $article_id]);
