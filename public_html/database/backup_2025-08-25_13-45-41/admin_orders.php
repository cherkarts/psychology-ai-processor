        ...
        <form method="GET" class="filters-form">
            <div class="filter-group">
                ...
                <select name="status" id="status" onchange="this.form.submit()">
                    ...
                    <option value="processing" <?php echo $status === 'processing' ? 'selected' : ''; ?>>Обрабатываются</option>
            ...
            <div class="filter-group">
                ...
                <input type="text" name="search" id="search" value="<?php echo sanitizeOutput($search); ?>" placeholder="Поиск по номеру заказа, email...">
                            ...
                            <tr class="order-row">
                                ...
                                <td class="order-actions">
                                    ...
                                    <?php if ($order['status'] === 'pending'): ?>
                                        <button class="btn btn-sm btn-success" onclick="updateOrderStatus('<?php echo $order['id']; ?>', 'processing')">
                                            <i class="fas fa-play"></i> Обработать
                                        </button>
                                    <?php elseif ($order['status'] === 'processing'): ?>
                                        <button class="btn btn-sm btn-success" onclick="updateOrderStatus('<?php echo $order['id']; ?>', 'completed')">
        ...
        <div class="pagination-wrapper">
            <nav class="pagination">
                <?php if ($orders['pagination']['current_page'] > 1): ?>
                    <a href="?page=<?php echo $orders['pagination']['current_page'] - 1; ?>&status=<?php echo $status; ?>&search=<?php echo urlencode($search); ?>" class="page-link">
    ...
    if ($db) {
        ...
        if (file_exists($ordersFile)) {
            ...
            foreach ($allOrders as $order) {
                if (($order['status'] ?? 'pending') === 'pending') $stats['pending']++;
