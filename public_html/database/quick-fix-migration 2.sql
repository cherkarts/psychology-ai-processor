-- Быстрое исправление недостающих элементов

-- 1. Создаем таблицу comments (если не существует)
...
-- 2. Создаем таблицу comment_likes (если не существует)
...
-- 3. Создаем таблицу comment_reports (если не существует)
...
-- 4. Добавляем поле telegram_user_id в таблицу reviews (если не существует)
ALTER TABLE `reviews` 
ADD COLUMN IF NOT EXISTS `telegram_user_id` BIGINT NULL AFTER `email`;

-- 5. Добавляем индекс для telegram_user_id (если не существует)
CREATE INDEX IF NOT EXISTS idx_reviews_telegram_user_id ON `reviews` (`telegram_user_id`);
