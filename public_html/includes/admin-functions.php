...
if (!function_exists('getArticleById')) {
    function getArticleById($slug)
    {
        ...
        // Пробуем разные способы подключения к БД
        if (!$db) {
            $db = getAdminDB();
        }

        // Если getAdminDB не работает, пробуем getDB
        ...
        if (!$db) {
            try {
                ...
                foreach ($charsets as $charset) {
                    try {
                        $dsn = "mysql:host={$config['database']['host']};dbname={$config['database']['dbname']};charset={$charset}";
                        $db = new PDO($dsn, $config['database']['username'], $config['database']['password'], $config['database']['options']);
                        error_log("Direct DB connection successful with charset: " . $charset);
                        break;
                    } catch (Exception $e) {
                        error_log("Failed with charset {$charset}: " . $e->getMessage());
                        continue;
                    }
                }
            } catch (Exception $e) {
                error_log("All DB connection methods failed: " . $e->getMessage());
                return false;
            }
        }
        ...
        try {
            ...
            if ($result) {
                error_log("Article found: " . $result['title']);
            } else {
...
if (!function_exists('saveArticle')) {
    function saveArticle($data, $files = [])
    {
        ...
        try {
            ...
            error_log("SaveArticle Debug - Title: " . $title);
            error_log("SaveArticle Debug - Content length: " . strlen($content));
            error_log("SaveArticle Debug - Article ID: " . ($data['article_id'] ?? 'new'));
            ...
            error_log("SaveArticle - Tags input: " . $tagsInput);
            error_log("SaveArticle - Meta keywords input: " . $metaKeywordsInput);

            $tagItems = [];

            // Проверяем не пришёл ли уже JSON
            $isJson = is_string($tagsInput) && !empty($tagsInput) &&
                null !== json_decode($tagsInput, true) && json_last_error() === JSON_ERROR_NONE;

            if ($isJson) {
                // Если пришёл JSON, декодируем его
                $tagItems = json_decode($tagsInput, true);
                error_log("SaveArticle - Tags from JSON: " . print_r($tagItems, true));
            } elseif (is_string($tagsInput) && trim($tagsInput) !== '') {
                // Если строка через запятую
                $tagItems = array_map('trim', explode(',', $tagsInput));
                error_log("SaveArticle - Tags from comma-separated: " . print_r($tagItems, true));
            }

            // Добавляем meta_keywords только если они не дублируют tags
            ...
            // Удаляем дубликаты, пустые значения и приводим к правильному виду
            $tagItems = array_values(array_unique(array_filter($tagItems, fn($v) => is_string($v) && trim($v) !== '')));
            error_log("SaveArticle - Tags final array: " . print_r($tagItems, true));

            $tags = !empty($tagItems) ? json_encode($tagItems, JSON_UNESCAPED_UNICODE) : null;
            error_log("SaveArticle - Tags final JSON: " . $tags);
            ...
            // Проверяем уникальность slug только для новых статей
            $isEdit = !empty($data['article_id']);
            if (!$isEdit) {
                // Только для новых статей проверяем уникальность
                $stmt = $db->prepare("SELECT id FROM articles WHERE slug = ?");
                $stmt->execute([$slug]);

                if ($stmt->rowCount() > 0) {
                    // Добавляем только короткий суффикс для новых статей
                    $counter = 1;
                    $originalSlug = $slug;
                    do {
                        ...
                        $stmt->execute([$slug]);
                        $counter++;
                    } while ($stmt->rowCount() > 0 && $counter < 100);
                }
            }
...
if (!function_exists('getProductById')) {
    function getProductById($id)
    {
        ...
        // Устанавливаем правильную кодировку для БД
...
if (!function_exists('saveProduct')) {
    function saveProduct($data, $files = [])
    {
        ...
        try {
            ...
            // Отладочная информация
            error_log('saveProduct - description length: ' . strlen($description));
            error_log('saveProduct - description preview: ' . substr($description, 0, 100));
            $content = $data['content'] ?? '';
            ...
            if (isset($data['old_price']) && $data['old_price'] !== '') {
                $oldPriceRaw = (string) $data['old_price'];
                $oldPrice = floatval(str_replace(',', '.', $oldPriceRaw));
            }
            ...
            // Badges (checkboxes) → JSON
            $badgesJson = null;
            if (!empty($data['badges']) && is_array($data['badges'])) {
                $badges = array_values(array_filter($data['badges'], fn($v) => $v !== ''));
                if (!empty($badges)) {
            ...
            if (isset($data['product_id']) && !empty($data['product_id'])) {
                ...
                // Отладочная информация
                error_log('SQL UPDATE executed: ' . ($result ? 'SUCCESS' : 'FAILED'));
                error_log('SQL query: ' . $sql);
                error_log('SQL params count: ' . count($params));

                return ['success' => true, 'message' => 'Товар успешно обновлен', 'slug' => $slug];
            } else {
                // Создание нового товара
                $stmt = $db->prepare("
                    INSERT INTO products (title, slug, short_description, description, type, status, category_id, price, old_price, in_stock, quantity, image, download_file, gallery, features, tags, badges, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
                ");
                $result = $stmt->execute([
                    $title,
                ...
                // Отладочная информация
                error_log('SQL INSERT executed: ' . ($result ? 'SUCCESS' : 'FAILED'));
...
if (!function_exists('getReviews')) {
    function getReviews($page = 1, $status = 'all', $search = '')
    {
        ...
        try {
            ...
            if (!empty($search)) {
                $conditions[] = "(name LIKE ? OR text LIKE ? OR email LIKE ?)";
                $searchTerm = "%{$search}%";
                $params[] = $searchTerm;
                $params[] = $searchTerm;
                $params[] = $searchTerm;
