...
console.log('article-enhancements.js: Script loaded')

document.addEventListener('DOMContentLoaded', function () {
  console.log('Article enhancements: DOMContentLoaded')
  ...
  function initReadabilityEnhancements() {
    ...
    const articleContent = document.querySelector('.article-content')
    console.log(
      'initReadabilityEnhancements: articleContent found:',
      !!articleContent
    )
    if (!articleContent) {
      console.log(
        'initReadabilityEnhancements: .article-content not found, returning'
      )
      return
    }
    ...
    console.log(
      'initReadabilityEnhancements: found',
      fontSizeBtns.length,
      'font size buttons'
    )
    fontSizeBtns.forEach((btn, index) => {
      console.log(
        'initReadabilityEnhancements: adding click listener to button',
        index,
        'with size:',
        btn.dataset.size
      )
      btn.addEventListener('click', function () {
        ...
        console.log('Font size button clicked, size:', size)
        setFontSize(size)
  ...
  function setFontSize(size) {
    ...
    const articleContent = document.querySelector('.article-content')
    console.log('setFontSize: articleContent found:', !!articleContent)
    if (!articleContent) {
      console.log('setFontSize: .article-content not found, returning')
      return
    }
    ...
    console.log('setFontSize: setting font-size to:', sizes[size])

    // Принудительно переопределяем любые внешние стили
    articleContent.style.setProperty('font-size', sizes[size], 'important')

    // Дополнительно переопределяем все возможные селекторы
    const style = document.createElement('style')
    style.id = 'dynamic-font-size-override'
    style.textContent = `
      .article-content {
        font-size: ${sizes[size]} !important;
      }
      .article-content * {
    ...
    const existingStyle = document.getElementById('dynamic-font-size-override')
    if (existingStyle) {
    ...
    articleContent.style.backgroundColor = 'rgba(255, 255, 0, 0.1)'
    setTimeout(() => {
      articleContent.style.backgroundColor = ''
    }, 500)

    console.log('setFontSize: font-size set successfully with CSS override')
  }
