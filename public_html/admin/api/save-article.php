...
// Проверяем API ключ в заголовке Authorization
$headers = getallheaders();
$authHeader = $headers['Authorization'] ?? $headers['authorization'] ?? '';
if (strpos($authHeader, 'Bearer ') === 0) {
  $apiKey = substr($authHeader, 7);
  $config = require '../../config.php';
  $expectedKey = $config['ai_api_key'] ?? 'default-ai-key';
  if ($apiKey === $expectedKey) {
    $isApiKeyAuth = true;
  }
}

// Если нет API ключа, проверяем сессию
if (!$isApiKeyAuth && !isset($_SESSION['admin_user'])) {
