...
СТРУКТУРА СТАТЬИ (ТОЛЬКО ЗАГОЛОВКИ РАЗДЕЛОВ):
...
ВАЖНО: НЕ добавляй **Описание:**, **Статья:**, **Краткое описание:**, "Размер текста", "A-", "A", "A+"
...
СТРУКТУРА СТАТЬИ (БЕЗ ЛИШНИХ ЭЛЕМЕНТОВ):
- Заголовки разделов: **Введение**, **Анализ причин**, **Практические техники**, **Профилактика и выводы**, **Часто задаваемые вопросы**
- НЕ добавляй **Описание:**, **Статья:**, **Краткое описание:**
- НЕ добавляй "Размер текста", "A-", "A", "A+"
- Каждый раздел должен быть четко структурирован
...
"""
    ...
    def format_article_content(self, content: str, title: str) -> Dict[str, str]:
        ...
        try:
            ...
            # Убираем все **Описание:** блоки
            formatted_content = formatted_content.replace('**Описание:**', '')
            
            # Убираем **Статья: и другие лишние элементы
            formatted_content = formatted_content.replace('**Статья:**', '')
            formatted_content = formatted_content.replace('**Краткое описание:**', '')
            formatted_content = formatted_content.replace('Размер текста:', '')
            formatted_content = formatted_content.replace('A-', '')
            formatted_content = formatted_content.replace('A', '')
            formatted_content = formatted_content.replace('A+', '')
            
            # Заменяем заголовки на HTML-теги
